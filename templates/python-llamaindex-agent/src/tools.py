...
PROMPT_SUMMARIZE = PromptTemplate(
    ...
    "{scraped_data}\n"
    "---------------------\n"
    "Given the scraped contact information and no prior knowledge, summarize contact information in a concise and informative manner.\n"
...
async def call_contact_details_scraper(
    ...
    if deduplicate:
        ...
        columns = list(set(df.columns) - CONTACT_DETAILS_ACTOR_FIELDS)
        data = df.unique(subset=columns).to_dicts()
