...
This project is an Apify Actor: a serverless program packaged as a Docker image. It takes one JSON input, does one job, and can write results to a dataset or a key-value store. Entry point: `src/main.js`. Configuration: `.actor/actor.json`.
...
1. **Implement** in `src/main.js`. Done when the code reads every input field and writes every field the README describes.
...
- Handle the `aborting` event with `Actor.on('aborting', handler)`: persist state, then `await Actor.exit()`, so a stopped run ends quickly and cheaply.
