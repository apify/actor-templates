...
1. **Implement** in `src/main.js`. Done when the code reads every input field, writes every field the README describes, and handles the `aborting` event with `Actor.on('aborting', handler)`.
